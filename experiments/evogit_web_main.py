...
host_id = sys.argv[1]
llm_name = sys.argv[2]
remote_repo = "git@github.com-evogit-web:BillHuang2001/evogit_web.git"
stages_dir = "./design_doc/evogit_web_stages"
base_name = "stage"
STAGE = 0
...
def respond_extractor(response: str) -> ResponseContent:
    try:
        ...
        if filename != "None" and not filename.startswith("src/components/"):
            print(
                f"Warning: filename {filename} does not start with src/components/, setting filename to None."
            )
            filename = "None"
...
config = EvoGitConfig(
    ...
    device_map="auto",
    git_dir=f"/tmp/evogit/evogit_web_{llm_name}_{host_id}",
    eval_command=None,
    seed_file=None,
    filename=None,
    merge_prob=1,
